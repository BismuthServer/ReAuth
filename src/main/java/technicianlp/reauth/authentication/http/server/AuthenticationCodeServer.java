...
public final class AuthenticationCodeServer {
    ...
    public AuthenticationCodeServer(int port, String loginUrl, CompletableFuture<String> codeFuture, Executor executor) throws IOException, NoClassDefFoundError {
        ...
        ReAuth.getLog().info("Starting local endpoint");
        server.start();
        ReAuth.getLog().info("Started local endpoint");
    ...
    public final synchronized void stop(boolean immediate) {
        if (this.running) {
            this.running = false;
            if (!immediate) {
                ReAuth.getLog().info("About to stop local endpoint");
                try {
                    TimeUnit.SECONDS.sleep(1);
                } catch (InterruptedException exception) {
                    ReAuth.getLog().warn("Interrupted while waiting to stop local endpoint", exception);
                }
            }
            ReAuth.getLog().info("Stopping local endpoint");
            this.stopServer.accept(immediate);
            ReAuth.getLog().info("Stopped local endpoint");
        }
    }
}
