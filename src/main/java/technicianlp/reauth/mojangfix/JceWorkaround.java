...
final class JceWorkaround {
    ...
    static void ensureUnlimitedCryptography() {
        try {
            if (Cipher.getMaxAllowedKeyLength("AES") != Integer.MAX_VALUE) {
                ReAuth.getLog().warn("Cryptography is restricted in this Java installation");
                if (!MojangJavaFix.java8) {
                    ReAuth.getLog().warn("Cryptography is likely deliberately restricted!");
                }
                removeCryptographyRestrictions();
                if (Cipher.getMaxAllowedKeyLength("AES") != Integer.MAX_VALUE) {
                    ReAuth.getLog().error("Failed to remove cryptography restriction");
                } else {
                    ReAuth.getLog().info("Cryptography restriction removed successfully");
                }
            }
    ...
    private static void removeCryptographyRestrictions() {
        try {
            ...
            ReAuth.getLog().error("Exception removing cryptography restrictions", e);
        }
    }
}
